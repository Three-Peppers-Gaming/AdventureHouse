...
namespace AdventureHouse.Services
{
    public class AdventureFrameworkService : IPlayAdventure
    {
            ...
            if (p.Player.PlayerDead == false)
            {
                ...
                if (movecommands.Contains(cs.Command))
                {
                    ...
                    if (cs.Valid == true)
                    {
                        ...
                        var monsterMessage = Helper_CheckForMonsters(p);
                        if (!string.IsNullOrEmpty(monsterMessage))
                        {
                            cs.Message += monsterMessage + "\r\n";
                        }
        ...
        private static string Helper_CheckForMonsters(PlayAdventure p)
        {
            var roomMonsters = p.Monsters.Where(m => m.RoomNumber == p.Player.Room).ToList();
            var messages = new List<string>();
            var random = new Random();

            foreach (var monster in roomMonsters)
            {
                // Only try to spawn monster if it's not already present and not dead
                if (!monster.IsPresent && monster.CurrentHealth > 0)
                {
                    // Check if monster should appear based on appearance chance
                    if (random.Next(1, 101) <= monster.AppearanceChance)
                    {
                        monster.IsPresent = true;
                        monster.CurrentHealth = monster.AttacksToKill; // Reset monster health
                        messages.Add(GetFunMessage(p.Messages, "MonsterAppear", monster.Name));
                    }
                }

                // If monster is present and alive, try to attack player
                if (monster.IsPresent && monster.CurrentHealth > 0)
                {
                    if (monster.CanHitPlayer && random.Next(1, 101) <= monster.HitOdds)
                    {
                        p.Player.HealthCurrent -= monster.HealthDamage;
                        messages.Add(GetFunMessage(p.Messages, "MonsterHit", monster.Name) + $" (Lost {monster.HealthDamage} health)");
                    }
                    else if (monster.CanHitPlayer)
                    {
                        messages.Add(GetFunMessage(p.Messages, "MonsterMiss", monster.Name));
                    }
                }
            }
        ...
        private static Tuple<PlayAdventure, CommandState> Helper_AttackMonster(PlayAdventure p, CommandState cs)
        {
            var targetName = cs.Modifier.ToUpper();
            var roomMonsters = p.Monsters.Where(m => m.RoomNumber == p.Player.Room && m.IsPresent && m.CurrentHealth > 0).ToList();

            // First check if there are any monsters present
            if (!roomMonsters.Any())
            {
                cs.Message = GetFunMessage(p.Messages, "NoMonster", "");
                cs.Valid = false;
                return new Tuple<PlayAdventure, CommandState>(p, cs);
            }

            // Try to find the target monster by name or key
            var targetMonster = roomMonsters.FirstOrDefault(m =>
                m.Name.ToUpper() == targetName ||
                m.Key.ToUpper() == targetName);

            if (targetMonster == null)
            {
                // If no specific monster targeted, use the first one available
                targetMonster = roomMonsters.First();
            }

            // Now find the required weapon for this monster in player's inventory
            var requiredWeapon = p.Items.FirstOrDefault(i =>
                i.Name.ToUpper() == targetMonster.ObjectNameThatCanAttackThem.ToUpper() &&
                i.Location == 9999);

            if (requiredWeapon != null)
            {
                // Player has the correct weapon - proceed with attack
                targetMonster.CurrentHealth--;
                cs.Message = $"You successfully attack the {targetMonster.Name} with your {requiredWeapon.Name}!";

                if (targetMonster.CurrentHealth <= 0)
                {
                    // Monster is defeated - reset for future spawning
                    ...
                    // Award points for defeating monster
                    p.Player = Helper_SetPlayerPoints(true, $"Defeated{targetMonster.Name}", p);
                }
        ...
        private static string Helper_GetMonsterRoomDescription(PlayAdventure p)
        {
            var roomMonsters = p.Monsters.Where(m => m.RoomNumber == p.Player.Room && m.IsPresent && m.CurrentHealth > 0).ToList();
            if (roomMonsters.Any())
            {
        ...
        private GameMoveResult Main_PostActionUpdate(GameMoveResult gmr, PlayAdventure p, string actionMessage)
        {
            ...
            // Add monster descriptions if any are present in the room
            var monsterDesc = Helper_GetMonsterRoomDescription(p);
            if (!string.IsNullOrEmpty(monsterDesc))
            {
                gmr.RoomMessage += monsterDesc + "\r\n";
            }
        ...
        private Tuple<PlayAdventure, CommandState> Action_Processing(PlayAdventure p, CommandState cs)
        {
            ...
            if (cs.Command == "attack") { (p, cs) = Helper_AttackMonster(p, cs); }
        ...
        private Tuple<PlayAdventure, CommandState> Action_UseItem(PlayAdventure p, CommandState cs)
        {
            ...
            if ((item != null) & (!Helper_IsPlayerDead(p)) && item.Location == 9999)
            {
                if (item.ActionVerb.ToLower() == cs.Command.ToLower())
                {
                    ...
                    if (item.ActionResult.ToLower() == "weapon")
                    {
                        // Handle weapon items (like flyswatter attacking mosquito)
                        var roomMonsters = p.Monsters.Where(m => m.RoomNumber == p.Player.Room && m.IsPresent && m.CurrentHealth > 0).ToList();
                        var targetMonster = roomMonsters.FirstOrDefault(m => m.ObjectNameThatCanAttackThem.ToUpper() == item.Name.ToUpper());

                        if (targetMonster != null)
                        {
                            targetMonster.CurrentHealth--;
                            cs.Message = $"You successfully attack the {targetMonster.Name} with your {item.Name}!";
                        ...
                        {
                            cs.Message = GetFunMessage(p.Messages, "AttackFailed", "monster");
                        }
                ...
                {
                    cs.Valid = false;
                    cs.Message = GetFunMessage(p.Messages, "UseFailed", cs.Modifier) + "\r\n";
                }
            }
            else
            {
                ...
                if (Helper_IsPlayerDead(p))
                {
                    cs.Message = "Dead people don't need stuff";
                }
                if ((item is not null) && (item.Location != 9999))
                {
                    cs.Valid = false;
                    cs.Message = GetFunMessage(p.Messages, "UseFailed", cs.Modifier) + "\r\n";
                }
