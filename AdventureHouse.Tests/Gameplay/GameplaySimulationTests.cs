...
namespace AdventureHouse.Tests.Gameplay
{
    ...
    public class GameplaySimulationTests
    {
        ...
        public void SpaceStation_ShouldHaveDifferentGameContent()
        {
            ...
            var adventureHouse = _server.PlayGame(adventureHouseRequest);
            var spaceStation = _server.PlayGame(spaceStationRequest);

            // Assert - Games should have different content
            Assert.NotEqual(adventureHouse.RoomName, spaceStation.RoomName);
            Assert.NotEqual(adventureHouse.MapData?.RenderingConfig.GameName, 
                           spaceStation.MapData?.RenderingConfig.GameName);
            Assert.NotEqual(adventureHouse.MapData?.CurrentLevelDisplayName, 
