...
using AdventureRealms.Services.AdventureServer.GameManagement;
...
namespace AdventureRealms.Services.AdventureServer
{
    ...
    public class AdventureFrameworkService : IPlayAdventure
    {
        ...
        /// </summary>
        public AdventureRealms.Services.Shared.Models.GamePlayResponse PlayGame(AdventureRealms.Services.Shared.Models.GamePlayRequest request)
        {
            try
            {
            ...
            {
                return new AdventureRealms.Services.Shared.Models.GamePlayResponse
                {
                    SessionId = "-1",
        ...
        /// </summary>
        private AdventureRealms.Services.Shared.Models.GamePlayResponse CreateNewGameSession(AdventureRealms.Services.Shared.Models.GamePlayRequest request)
        {
            // Validate game ID
            var games = GameList();
            ...
            // Get welcome message from game configuration
            var welcomeMessage = gameConfig.GetWelcomeMessage(playAdventure.Player.Name);
            
            // Room description should NOT include welcome message anymore
            var roomDescription = room.Desc;
            ...
            return new AdventureRealms.Services.Shared.Models.GamePlayResponse
            {
                ...
                WelcomeMessage = welcomeMessage, // Send welcome message separately for popup display
        ...
        /// </summary>
        private AdventureRealms.Services.Shared.Models.GamePlayResponse ProcessGameCommand(AdventureRealms.Services.Shared.Models.GamePlayRequest request)
        {
            // Validate session
        ...
        /// </summary>
        private AdventureRealms.Services.Shared.Models.GamePlayResponse ProcessGameCommandInternal(AdventureRealms.Services.Shared.Models.GamePlayRequest request, CommandState commandState, PlayAdventureModel playAdventure, ClientSessionState sessionState)
        {
            var room = _roomManagementService.GetRoom(playAdventure.Rooms, playAdventure.Player.Room);
            
            var response = new AdventureRealms.Services.Shared.Models.GamePlayResponse
            {
                SessionId = request.SessionId,
                GameName = playAdventure.GameName,
                RoomName = room.Name,
        ...
        private bool DoesRoomHaveItems(int roomNumber, List<AdventureRealms.Services.AdventureServer.Models.Item> items)
        {
            return items.Any(item => item.Location == roomNumber);
        }

        private List<RoomConnection> GetDiscoveredConnections(AdventureRealms.Services.AdventureServer.Models.Room room, AdventureClient.Models.MapState mapState)
        {
